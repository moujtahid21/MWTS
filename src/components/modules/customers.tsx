...
export function Customers() {
  ...
  const [list, setList] = useState(D.customers);
  const [q, setQ] = useState("");
  const [edit, setEdit] = useState(null); // customer obj or "new"
  ...
  const save = (data) => {
    if (data.id && list.some(c => c.id === data.id)) setList(l => l.map(c => c.id === data.id ? data : c));
    else setList(l => [{ ...data, id: "K-" + (1100 + l.length), orders: 0, openOrders: 0 }, ...l]);
    setEdit(null); toast("Kunde gespeichert", "check");
  };
  const del = (id) => { setList(l => l.filter(c => c.id !== id)); toast("Kunde gelöscht", "trash"); };
  ...
  return (
    <div className="view-narrow">
      ...
      {view === "grid" ? (
            ...
            <div key={c.id} className="card" style={{ display: "flex", flexDirection: "column" }}>
              <div className="card-pad" style={{ flex: 1 }}>
                <div className="flex items-center gap-sm" style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}><div className="t-strong" style={{ fontSize: 14.5, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.name}</div><div className="t-mut t-mono" style={{ fontSize: 11.5 }}>{c.id}</div></div>
        ...
        <div className="card"><div className="tbl-wrap"><table className="tbl">
          ...
          <tbody>
            {filtered.map(c => (
              <tr key={c.id}>
                <td><div className="flex items-center gap-sm"><div className="avatar" style={{ width: 32, height: 32, flexBasis: 32, borderRadius: 8, background: "var(--color-primary-soft)", color: "var(--color-primary-strong)" }}><Icon name="building" size={16} /></div><div><div className="t-strong">{c.name}</div><div className="t-mut t-mono" style={{ fontSize: 11 }}>{c.id}</div></div></div></td>
      ...
      {edit && <CustomerForm D={D} customer={edit === "new" ? null : edit} onClose={() => setEdit(null)} onSave={save} />}
    </div>
  );
}
...
function CustomerForm({ customer, onClose, onSave }) {
  ...
  return (
    <Modal size="lg" title={customer ? "Kunde bearbeiten" : "Neuer Kunde"} sub={customer ? customer.id : "Auftraggeber-Stammdaten"} onClose={onClose}
      footer={<React.Fragment><div style={{ flex: 1 }} /><button className="btn" onClick={onClose}>Schließen</button><button className="btn btn-primary" disabled={!valid} onClick={() => onSave(f)}><Icon name="check" size={16} />Speichern</button></React.Fragment>}>
